...
## 关键决策记录（勿无意推翻）
- **视频不持久化，账号数据持久化**: 视频/字幕 JSON 不挂对象存储，工作流以本地文件为主（识别完自动下载视频+JSON，SRT 由句子列表"保存"按钮获取）。`commerce.db`（用户账号/积分/使用记录）已挂 Railway Volume 持久化至 `/data/commerce.db`，重新部署不丢失。环境变量 `COMMERCE_DB_PATH=/data/commerce.db`。
...
- **首页服务器视频列表已隐藏**: 产品转向多用户后，服务器列表不区分用户（隐私问题），且 Railway 文件系统重部署即清空，本地视频列表已覆盖全部使用场景，故前端不再调用 `loadVideoList()`，`myVideosSection` 隐藏。后端 `/api/videos` 接口暂时保留。
