"""
Free 套餐 Rate Limiter（Task 5.2）

基于内存计数，重启清零（对免费用户可接受）。
...
公开接口：
  ...
  increment(user_id, capability)                  → int   当日累计次数
  get_usage(user_id, capability)                  → int   当日已使用次数
